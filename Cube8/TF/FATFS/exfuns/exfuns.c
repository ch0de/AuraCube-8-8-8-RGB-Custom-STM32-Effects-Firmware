...
FIL *file;	  		//文件1
FIL *htmlFile;	  		//文件1
FIL *file1;	 // Not used - For CUBE12 ONLY  		
FIL *file2;	  		
FIL *file3;	  		
FIL *file4;	  		
FIL *file5;	
...
u8 exfuns_init(void)
{
	...
	htmlFile = (FIL*)mymalloc(sizeof(FIL));			//为file申请内存
	fatbuf = (u8*)mymalloc(512);		//为fatbuf申请内存 //only need 384 but with Flash we need 4096 // just cheat and don't allocate it all - Hope it doesnt crash
	if (fs[0] && file && htmlFile && fatbuf)
